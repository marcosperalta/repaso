...
int main(int argc, char *argv[]) {
    ...
    /* reset counters and set time for selection_sort */
    reset_comparisons_counter();
    reset_swaps_counter();
    set_current_time();

    /* do the actual sorting for selection_sort */
    selection_sort(copy, length);

    /* show statistics for selection_sort */
    printf("Statistics for selection_sort\n");
    printf("Time elapsed: %g, Comparisons: %u, Swaps: %u\n", calculate_elapsed_time(), comparisons_number(), swaps_number());

    /* Reset counters and set time for insertion_sort */
    reset_comparisons_counter();
    reset_swaps_counter();
    set_current_time();

    /* create a copy of the array for insertion_sort */
    int insertion_copy[MAX_SIZE];
    array_copy(insertion_copy, array, length);

    /* do the actual sorting for insertion_sort */
    insertion_sort(insertion_copy, length);

    /* show statistics for insertion_sort */
    printf("\nStatistics for insertion_sort\n");
    printf("Time elapsed: %g, Comparisons: %u, Swaps: %u\n", calculate_elapsed_time(), comparisons_number(), swaps_number());

    /* Reset counters and set time for quick_sort */
    reset_comparisons_counter();
    reset_swaps_counter();
    set_current_time();

    /* create a copy of the array for quick_sort */
    int quick_copy[MAX_SIZE];
    array_copy(quick_copy, array, length);

    /* do the actual sorting for quick_sort */
    quick_sort(quick_copy, (length == 0u) ? 0u : length - 1u);
